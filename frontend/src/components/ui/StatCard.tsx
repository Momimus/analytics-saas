  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs uppercase tracking-[0.18em] text-[var(--ui-text-subtle)]">{label}</p>
        {icon && <span className="text-[var(--ui-text-muted)]">{icon}</span>}
      </div>
      <p className="mt-2 text-3xl font-semibold tracking-tight text-[var(--text)]">{value}</p>
      {hint && <p className="mt-1 text-xs text-[var(--ui-text-muted)]">{hint}</p>}
    </article>
  );
}
